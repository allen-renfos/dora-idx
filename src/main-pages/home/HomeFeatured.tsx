...
export default function HomeFeatured() {
  ...
  return (
    <section className="relative bg-[var(--surface-obsidian)] text-white section-pad overflow-hidden">
      {/* Decorative gradient */}
      <div
        aria-hidden
        className="absolute -top-40 right-0 w-[520px] h-[520px] rounded-full opacity-[0.06] blur-[120px] bg-[var(--gold-500)]"
      />
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="flex flex-col h-full bg-[var(--surface-charcoal)] border border-white/5 rounded-sm overflow-hidden animate-pulse"
                >
                  <div className="aspect-[4/3] bg-white/5" />
                  <div className="p-6 flex flex-col gap-3">
                    <div className="h-7 w-40 bg-white/10 rounded-sm" />
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center text-center py-20 border border-dashed border-white/10">
      ...
      <h3 className="font-serif text-2xl">No new listings right now</h3>
      <p className="text-white/60 mt-2 max-w-md">
        Check back shortly, or{" "}
        <Link href="/properties" className="text-[var(--gold-500)] underline underline-offset-4">
